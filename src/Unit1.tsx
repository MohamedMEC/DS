...
function PythonPanel({ chapter }: { chapter: Chapter }) {
  ...
  return <section className="u1-python"><div><div><Code2/><span><b>NumPy connection</b>See how the visual experiment is expressed in Python.</span></div><button onClick={()=>setOpen(v=>!v)}>{open?"Hide code":"Open code lab"}<ChevronDown/></button></div>{open&&<div className="u1-python-body"><div className="u1-code-window"><header><i/><i/><i/><span>unit_1_lab.py</span></header><pre><code>import numpy as np{"\n\n"}{sample.code}</code></pre></div><div className="u1-run-panel"><button onClick={()=>setRan(true)}><Play/>Run example</button><span>Expected output</span><pre>{ran?sample.output:"Click Run example to reveal and check the result."}</pre></div></div>}</section>;
}
