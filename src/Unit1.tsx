...
function VectorLab() {
  ...
  return <LabShell title="Build, add and compare two vectors" goal="Watch v₁ drawn from the origin, then a translated copy of v₂ starting at v₁'s tip — walking both legs lands you at v₁+v₂.">
    <div className="u1-lab-grid"><div className="u1-controls"><div className="u1-control-pair"><NumberBox label="v₁.x" value={ax} onChange={setAx}/><NumberBox label="v₁.y" value={ay} onChange={setAy}/><NumberBox label="v₂.x" value={bx} onChange={setBx}/><NumberBox label="v₂.y" value={by} onChange={setBy}/></div><div className="u1-stats"><Stat label="‖v₁‖" value={f(magA)}/><Stat label="v₁·v₂" value={f(dot)}/><Stat label="Angle between v₁ and v₂" value={`${f(angle,1)}°`}/><Stat label="v₁+v₂" value={`[${f(ax+bx)}, ${f(ay+by)}]`}/></div></div>
      ...
      </Plane><div className="u1-legend"><span className="v1">v₁</span><span className="v2">v₂ (translated)</span><span className="sum">v₁+v₂</span>{showArc&&<span className="angle">∠ = {f(angle,1)}°</span>}</div></div></div>
    <div className="u1-observation"><Eye/><p><b>Observe:</b> [{f(ax)},{f(ay)}] + [{f(bx)},{f(by)}] = [{f(ax+bx)},{f(ay+by)}] — walk v₁ from the origin, then walk a translated copy of v₂ starting at v₁'s tip, and you land on the same point as the green diagonal v₁+v₂. The dashed lines complete the parallelogram, showing v₁+v₂ = v₂+v₁ either way round. The dot product is {f(dot)} and the angle between v₁ and v₂ is {f(angle,1)}° — {Math.abs(dot)<.001?"the vectors are perpendicular.":dot>0?"they point generally in the same direction.":"they point generally in opposite directions."}</p></div>
  </LabShell>;
}
...
function GradientLab(){
  ...
  return <LabShell title="Train the line one step at a time" goal="Gradient descent starts with a poor line and repeatedly reduces error instead of using the closed-form solution."><div className="u1-presets"><button onClick={()=>advance(1)}>Take 1 step</button><button onClick={()=>advance(10)}>Take 10 steps</button><button onClick={()=>advance(100)}>Take 100 steps</button><button onClick={()=>{setM(0);setB(0);setSteps(0)}}><RotateCcw/>Reset</button></div><div className="u1-lab-grid"><div className="u1-controls"><MiniRange label="learning rate α" value={alpha} min={.001} max={.16} step={.001} onChange={setAlpha}/><div className="u1-stats"><Stat label="Steps taken" value={`${steps}`}/><Stat label="Current slope" value={f(m)}/><Stat label="Current intercept" value={f(b)}/><Stat label="Current SSE" value={f(sse)} good={sse<3}/></div><div className="u1-equation-stack"><span>m ← m − α × gradientₘ</span><span>b ← b − α × gradientᵦ</span><span>target solution ≈ [{f(best.b)}, {f(best.m)}]</span></div></div><div className="u1-ls-chart"><svg viewBox="0 0 420 270" role="img" aria-label={`Scatter plot with the line after ${steps} gradient descent steps, sum of squared errors ${f(sse)}`}>{[0,1,2,3,4,5].map(i=><line key={`v${i}`} x1={px(i)} y1="20" x2={px(i)} y2="245" className="u1-grid"/>)}{[0,2,4,6,8,10].map(i=><line key={`h${i}`} x1="42" y1={py(i)} x2="390" y2={py(i)} className="u1-grid"/>)}<line x1={px(0)} y1={py(b)} x2={px(4.5)} y2={py(m*4.5+b)} className="u1-fit-line"/>{points.map((p,i)=><circle key={i} cx={px(p.x)} cy={py(p.y)} r="6" className="u1-data-point"/>)}</svg></div></div><div className="u1-observation"><LineChart/><p><b>Optimisation:</b> {sse<3?"the line is near the minimum. Smaller later steps can refine it.":alpha>.11?"a large learning rate may jump around or overshoot. Try a smaller value.":"take more steps and watch SSE fall toward the least-squares minimum."}</p></div></LabShell>
}
...
function MissionPanel({ chapter, completed, toggle }: { chapter: Chapter; completed: string[]; toggle: (key: string) => void }) {
  return <section className="u1-missions"><div className="u1-mission-head"><div><Target/><span><b>Experiment missions</b>Do these inside the live lab—then tick only when you can explain what happened.</span></div><strong>{missions[chapter.id].filter((_,i)=>completed.includes(`${chapter.id}-${i}`)).length}/3</strong></div><div className="u1-mission-grid">{missions[chapter.id].map((m,i)=>{const key=`${chapter.id}-${i}`,done=completed.includes(key);return <button className={done?"done":""} aria-pressed={done} key={m} onClick={()=>toggle(key)}><i>{done?<Check aria-hidden="true"/>:i+1}</i><span>{m}</span></button>})}</div></section>;
}
...
function MisconceptionLab(){
  ...
  return <section className="u1-mistakes"><div className="u1-mistake-head"><div><CircleHelp/><span><b>Mistake detective</b>Decide whether each common statement is true or false.</span></div><strong>{answers.filter((a,i)=>a===misconceptions[i].truth).length}/{misconceptions.length} correct</strong></div><div className="u1-mistake-grid">{misconceptions.map((m,i)=><article key={m.s}><p>{m.s}</p><div><button className={answers[i]===true?(m.truth?"correct":"wrong"):""} aria-pressed={answers[i]===true} onClick={()=>setAnswers(v=>v.map((a,j)=>j===i?true:a))}>True</button><button className={answers[i]===false?(!m.truth?"correct":"wrong"):""} aria-pressed={answers[i]===false} onClick={()=>setAnswers(v=>v.map((a,j)=>j===i?false:a))}>False</button></div>{answers[i]!==null&&<small>{answers[i]===m.truth?<Check/>:<CircleHelp/>}{m.why}</small>}</article>)}</div></section>;
}
...
function ConceptCard({ chapter }: { chapter: Chapter }){
  ...
  return <section className="u1-concept"><div className="u1-concept-head"><span>{chapter.number}</span><div><p>GUIDED CONCEPT</p><h2>{chapter.title}</h2></div></div><div className="u1-step-tabs">{steps.map((s,i)=>{const I=s.icon;return <button key={s.label} className={step===i?"active":""} onClick={()=>setStep(i)}><I/><span>{i+1}. {s.label}</span></button>})}</div><div className="u1-step-body"><span>0{step+1}</span><div><h3>{steps[step].label}</h3><p>{steps[step].text}</p>{step===2&&<aside>Read every symbol from the example before trying to memorise the expression.</aside>}</div></div><div className="u1-check"><div><CircleHelp/><span><b>Quick check</b>{chapter.check.q}</span></div><div className="u1-check-options">{chapter.check.options.map((o,i)=><button className={choice===null?"":i===chapter.check.answer?"correct":choice===i?"wrong":""} aria-pressed={choice===i} key={o} onClick={()=>setChoice(i)}>{choice!==null&&i===chapter.check.answer?<Check aria-hidden="true"/>:<i aria-hidden="true">{String.fromCharCode(65+i)}</i>}{o}</button>)}</div>{choice!==null&&<p className={correct?"correct":"wrong"}>{correct?"Correct. ":"Not quite. "}{chapter.check.why}</p>}</div></section>
}
...
export default function Unit1Studio(){
  ...
  // across all 18 topics without needing Tab to step through every button.
