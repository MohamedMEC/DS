...
const chapters: Chapter[] = [
  {
    ...
    ml: "Every row of a machine-learning dataset is commonly treated as a feature vector. Vector magnitude is used in normalisation, distance calculations and regularisation (like the L2 penalty) — it is not generally a confidence score, though some specialised embedding models do encode information in vector norms.",
...
const missions: Record<string, string[]> = {
  vectors: ["Make the vector's length ‖v‖ exactly 5.", "Make the vector point at 45° from the x-axis.", "Normalise it and confirm the unit vector v̂ has length exactly 1."],
  "vector-addition": ["Make u+v equal exactly [4,3].", "Find two different pairs of u,v that both sum to [5,0].", "Confirm u+v equals v+u by swapping the number fields."],
  ...
  dot: ["Project u=[3,2] onto the horizontal axis.", "Choose u and v so the projection equals u.", "Confirm remainder·v is always zero, and explain why."],
...
function VectorLab() {
  ...
  const dot=ax*bx+ay*by, magA=Math.hypot(ax,ay), magB=Math.hypot(bx,by), hasBoth=magA>0.001&&magB>0.001;
  const angle=hasBoth?Math.acos(clamp(dot/(magA*magB),-1,1))*180/Math.PI:0;
  // Angle arc between u and v, drawn in screen space (svg y grows downward, so flip the y component).
  const showArc = hasBoth;
  ...
  return <LabShell title="Build, add and compare two vectors" goal="Watch u drawn from the origin, then a translated copy of v starting at u's tip — walking both legs lands you at u+v.">
    <div className="u1-lab-grid"><div className="u1-controls"><div className="u1-control-pair"><NumberBox label="u.x" value={ax} onChange={setAx}/><NumberBox label="u.y" value={ay} onChange={setAy}/><NumberBox label="v.x" value={bx} onChange={setBx}/><NumberBox label="v.y" value={by} onChange={setBy}/></div><div className="u1-stats"><Stat label="‖u‖" value={f(magA)}/><Stat label="u·v" value={f(dot)}/><Stat label="Angle between u and v" value={hasBoth?`${f(angle,1)}°`:"—"}/><Stat label="u+v" value={`[${f(ax+bx)}, ${f(ay+by)}]`}/></div></div>
      <div className="u1-visual"><Plane>
        <line x1={sx(bx)} y1={sy(by)} x2={sx(ax+bx)} y2={sy(ay+by)} className="u1-dashed-v1"/>
        <line x1={sx(0)} y1={sy(0)} x2={sx(bx)} y2={sy(by)} className="u1-dashed-v2"/>
        <line x1={sx(0)} y1={sy(0)} x2={sx(ax)} y2={sy(ay)} className="u1-v1" markerEnd="url(#arrow-purple)"/>
        <line x1={sx(ax)} y1={sy(ay)} x2={sx(ax+bx)} y2={sy(ay+by)} className="u1-v2" markerEnd="url(#arrow-coral)"/>
        <line x1={sx(0)} y1={sy(0)} x2={sx(ax+bx)} y2={sy(ay+by)} className="u1-vsum" markerEnd="url(#arrow-green)"/>
        {showArc&&<path d={`M ${arcX1} ${arcY1} A ${arcR} ${arcR} 0 0 ${arcSweep} ${arcX2} ${arcY2}`} className="u1-angle-arc"/>}{showArc&&<text x={labelX} y={labelY} textAnchor="middle" className="u1-angle-label">{f(angle,1)}°</text>}
      </Plane><div className="u1-legend"><span className="v1">u</span><span className="v2">v (translated)</span><span className="sum">u+v</span>{showArc&&<span className="angle">∠ = {f(angle,1)}°</span>}</div></div></div>
    <div className="u1-observation"><Eye aria-hidden="true"/><p><b>Observe:</b> [{f(ax)},{f(ay)}] + [{f(bx)},{f(by)}] = [{f(ax+bx)},{f(ay+by)}] — walk u from the origin, then walk a translated copy of v starting at u's tip, and you land on the same point as the green diagonal u+v. The dashed lines complete the parallelogram, showing u+v = v+u either way round. {hasBoth?<>The dot product is {f(dot)} and the angle between u and v is {f(angle,1)}° — {Math.abs(dot)<.001?"the vectors are perpendicular.":dot>0?"they point generally in the same direction.":"they point generally in opposite directions."}</>:<>One vector has zero length, so no angle or direction comparison is defined until both u and v are nonzero.</>}</p></div>
  </LabShell>;
}
...
function SpanLab(){
  ...
  return <LabShell title="Can these two vectors reach the target?" goal="Adjust c₁ and c₂ — or click Auto-solve — to land the green marker exactly on the target. That reachable set is the span.">
    <div className="u1-lab-grid"><div className="u1-controls">
      ...
      <div className="u1-presets"><button className="u1-preset" onClick={()=>{setBx(ax*2);setBy(ay*2)}}>Make v₂ parallel to v₁</button><button className="u1-preset" onClick={solve} disabled={!independent}><WandSparkles aria-hidden="true"/>Auto-solve for target</button></div>
    ...
    <div className={`u1-observation ${reached?"":!independent?"warn":""}`}><Target aria-hidden="true"/><p>{reached?<><b>Reached:</b> c₁={f(c1)} and c₂={f(c2)} land exactly on [{f(gx)}, {f(gy)}].</>:independent?<><b>Independent — every point is reachable:</b> the shaded plane and the sample dots show that some (c₁,c₂) pair can reach any target here. Try Auto-solve.</>:zeroBoth?<><b>Both vectors are zero:</b> the only reachable point is the origin.</>:<><b>Dependent — reach collapses to one line:</b> v₂ adds no new direction, so only points on the line shown can ever be reached, whatever c₁ and c₂ you pick — unless the target happens to sit exactly on that line.</>}</p></div>
  </LabShell>;
}
...
function SingleVectorLab(){
  ...
  return <LabShell title="Meet a vector: components, length and direction" goal="Change the components and watch the magnitude, unit vector and angle update together.">
    ...
    <div className="u1-observation"><Eye aria-hidden="true"/><p><b>Observe:</b> the unit vector always has length 1 and points in the exact same direction as v — dividing by the magnitude "normalises" it.</p></div>
  </LabShell>;
}
...
function SubtractLab(){
  ...
  return <LabShell title="Find the gap between two points" goal="Move u and v — the dashed connector from v's tip to u's tip is exactly u−v, and its length is the distance between them.">
    ...
    <div className="u1-observation"><Minus aria-hidden="true"/><p><b>Observe:</b> {dist<.01?"u and v are now the same point — the distance between them has dropped to 0.":`the dashed connector from v to u has the same length and direction as the green u−v vector: both equal ${f(dist)}.`}</p></div>
  </LabShell>;
}
...
function CrossProductLab(){
  ...
  return <LabShell title="Build a vector perpendicular to two others" goal="Edit two 3D vectors a and b — the green vector is a×b, always perpendicular to both (check the dot products below).">
    ...
    <div className="u1-observation"><Axis3D aria-hidden="true"/><p>{magC<.001?<><b>Zero vector:</b> a and b are parallel (or one of them is zero), so a×b = [0, 0, 0]. The zero vector has no direction, so there's nothing left to check for perpendicularity.</>:<><b>Perpendicularity check:</b> both dot products are 0 ({f(checkA)} and {f(checkB)}), confirming a×b is genuinely perpendicular to both a and b.</>}</p></div>
  </LabShell>;
}
...
function SystemsLab(){
  ...
  return <LabShell title="Solve Ax=b and see where the equations meet" goal="Toggle between a 2×2 system (two lines) and a 3×3 system (three planes) and watch the solution type follow the geometry.">
    <div className="u1-order-toggle"><button className={mode==="2d"?"active":""} aria-pressed={mode==="2d"} onClick={()=>setMode("2d")}><Equal aria-hidden="true"/> 2×2 (lines)</button><button className={mode==="3d"?"active":""} aria-pressed={mode==="3d"} onClick={()=>setMode("3d")}><Axis3D aria-hidden="true"/> 3×3 (planes)</button></div>
...
function ConditioningLab(){
  ...
  return <LabShell title="Amplify a tiny measurement error" goal="The matrix remains full rank, but near-parallel equations make the solution increasingly fragile."><div className="u1-conditioning"><div className="u1-controls"><MiniRange label="separation ε" value={eps} min={.01} max={2} step={.01} onChange={setEps}/><MiniRange label="tiny noise added to b₂" value={noise} min={0} max={.05} step={.001} onChange={setNoise}/><div className="u1-equation-stack"><span>A = [[1,1],[1,{f(1+eps)}]]</span><span>clean b = [2,{f(2+eps)}]</span><span>observed b = [2,{f(2+eps+noise)}]</span></div></div><div className="u1-condition-meter"><span>Condition number</span><strong>{cond>9999?">9999":f(cond,1)}</strong><div><i style={{width:`${Math.min(100,Math.log10(cond)*32)}%`}}/></div><small>{cond<10?"Well conditioned":cond<100?"Sensitive":"Highly unstable"}</small></div><div className="u1-solution-shift"><div><span>Clean solution</span><strong>[1, 1]</strong></div><ArrowRight aria-hidden="true"/><div><span>With tiny noise</span><strong>[{f(x1)}, {f(x2)}]</strong></div><small>Solution moved by {f(change)} even though b changed by only {f(noise,3)}</small></div></div><div className={`u1-observation ${cond>=100?"warn":""}`}><Gauge aria-hidden="true"/><p><b>Key lesson:</b> full rank tells us a unique solution exists. Conditioning tells us whether that solution is trustworthy under small noise.</p></div></LabShell>
}
...
function LeastSquaresLab(){
  ...
  return <LabShell title="Compete against the least-squares line" goal="Move slope and intercept, inspect every residual, then reveal the mathematical optimum."><div className="u1-presets"><button className={mode==="imperfect"?"active":""} aria-pressed={mode==="imperfect"} onClick={()=>setMode("imperfect")}>Imperfect data</button><button className={mode==="perfect"?"active":""} aria-pressed={mode==="perfect"} onClick={()=>{setMode("perfect");setM(2);setB(1)}}>Perfect data</button><button onClick={()=>{setM(Number(best.m.toFixed(2)));setB(Number(best.b.toFixed(2)))}}><WandSparkles aria-hidden="true"/> Show best fit</button></div><div className="u1-lab-grid"><div className="u1-controls"><MiniRange label="Slope m" value={m} min={0} max={4} step={.05} onChange={setM}/><MiniRange label="Intercept b" value={b} min={-2} max={4} step={.05} onChange={setB}/><div className="u1-big-equation">ŷ = {f(m)}x {b>=0?"+":"−"} {f(Math.abs(b))}</div><div className={`u1-score ${sse<.01?"perfect":""}`}><span>Sum of squared errors</span><strong>{f(sse)}</strong><small>Best possible here: {f(points.reduce((s,p)=>s+(p.y-(best.m*p.x+best.b))**2,0))}</small></div></div><div className="u1-ls-chart"><svg viewBox="0 0 420 270" role="img" aria-label={`Scatter plot with the current fitted line, sum of squared errors ${f(sse)}`}>{[0,1,2,3,4,5].map(i=><line key={`v${i}`} x1={px(i)} y1="20" x2={px(i)} y2="245" className="u1-grid"/>)}{[0,2,4,6,8,10].map(i=><line key={`h${i}`} x1="42" y1={py(i)} x2="390" y2={py(i)} className="u1-grid"/>)}<line x1={px(0)} y1={py(b)} x2={px(4.5)} y2={py(m*4.5+b)} className="u1-fit-line"/>{rows.map((r,i)=><g key={i}><line x1={px(r.x)} y1={py(r.y)} x2={px(r.x)} y2={py(r.hat)} className="u1-residual-line"/><circle cx={px(r.x)} cy={py(r.y)} r="6" className="u1-data-point"/></g>)}</svg></div></div><div className="u1-calc-table"><div><b>x</b><b>Actual y</b><b>Predicted ŷ</b><b>Residual e</b><b>e²</b></div>{rows.map((r,i)=><div key={i}><span>{r.x}</span><span>{f(r.y)}</span><span>{f(r.hat)}</span><span>{f(r.e)}</span><span>{f(r.e*r.e)}</span></div>)}<footer><b>Total SSE</b><strong>{f(sse)}</strong></footer></div><div className="u1-normal-equation"><Sigma aria-hidden="true"/><div><span>Matrix solution</span><strong>β̂ = (XᵀX)⁻¹Xᵀy = [{f(best.b)}, {f(best.m)}]</strong><small>The first number is intercept; the second is slope.</small></div></div></LabShell>
}

function GradientLab(){
  const points=[{x:1,y:3},{x:2,y:5.6},{x:3,y:6.2},{x:4,y:9.4}],[m,setM]=useState(0),[b,setB]=useState(0),[alpha,setAlpha]=useState(.01),[steps,setSteps]=useState(0),[diverged,setDiverged]=useState(false);const best=fitLine(points),sse=points.reduce((s,p)=>s+(p.y-(m*p.x+b))**2,0),px=(x:number)=>42+x*76,py=(y:number)=>245-y*20;
  const advance=(count:number)=>{let mm=m,bb=b,taken=0,blew=false;for(let j=0;j<count;j++){const errors=points.map(p=>mm*p.x+bb-p.y),gm=2/points.length*errors.reduce((s,e,i)=>s+e*points[i].x,0),gb=2/points.length*errors.reduce((s,e)=>s+e,0);mm-=alpha*gm;bb-=alpha*gb;taken++;if(!Number.isFinite(mm)||Math.abs(mm)>1e8){mm=0;bb=0;blew=true;break}}setM(mm);setB(bb);setSteps(v=>v+taken);setDiverged(blew)};
  return <LabShell title="Train the line one step at a time" goal="Gradient descent starts with a poor line and repeatedly reduces error instead of using the closed-form solution."><div className="u1-presets"><button onClick={()=>advance(1)}>Take 1 step</button><button onClick={()=>advance(10)}>Take 10 steps</button><button onClick={()=>advance(100)}>Take 100 steps</button><button onClick={()=>{setM(0);setB(0);setSteps(0);setDiverged(false)}}><RotateCcw aria-hidden="true"/>Reset</button></div><div className="u1-lab-grid"><div className="u1-controls"><MiniRange label="learning rate α" value={alpha} min={.001} max={.16} step={.001} onChange={setAlpha}/><div className="u1-stats"><Stat label="Steps taken" value={`${steps}`}/><Stat label="Current slope" value={f(m)}/><Stat label="Current intercept" value={f(b)}/><Stat label="Current SSE" value={f(sse)} good={sse<3}/></div><div className="u1-equation-stack"><span>m ← m − α × gradientₘ</span><span>b ← b − α × gradientᵦ</span><span>target solution ≈ [{f(best.b)}, {f(best.m)}]</span></div></div><div className="u1-ls-chart"><svg viewBox="0 0 420 270" role="img" aria-label={`Scatter plot with the line after ${steps} gradient descent steps, sum of squared errors ${f(sse)}`}>{[0,1,2,3,4,5].map(i=><line key={`v${i}`} x1={px(i)} y1="20" x2={px(i)} y2="245" className="u1-grid"/>)}{[0,2,4,6,8,10].map(i=><line key={`h${i}`} x1="42" y1={py(i)} x2="390" y2={py(i)} className="u1-grid"/>)}<line x1={px(0)} y1={py(b)} x2={px(4.5)} y2={py(m*4.5+b)} className="u1-fit-line"/>{points.map((p,i)=><circle key={i} cx={px(p.x)} cy={py(p.y)} r="6" className="u1-data-point"/>)}</svg></div></div><div className={`u1-observation ${diverged?"warn":""}`}><LineChart aria-hidden="true"/><p><b>Optimisation:</b> {diverged?"this learning rate diverged — the parameters blew up partway through the requested steps, so they were reset to [0, 0]. That's the instability the mission is asking you to find; try a smaller α to see it converge instead.":sse<3?"the line is near the minimum. Smaller later steps can refine it.":alpha>.11?"a large learning rate may jump around or overshoot. Try a smaller value.":"take more steps and watch SSE fall toward the least-squares minimum."}</p></div></LabShell>
}
...
function PseudoinverseLab(){
  ...
  const uLenRaw=Math.hypot(x1a,x1b),zeroU=uLenRaw<.01,uLen=uLenRaw||1,dirX=x1a/uLen,dirY=x1b/uLen;
  ...
  return <LabShell title="Project y onto a column space with the pseudoinverse" goal="X's second column is k times its first, so both columns share one line — the column space. X⁺ finds the exact point on that line closest to y; the leftover is always perpendicular to it (watch for the small right-angle mark).">
    <div className="u1-presets"><button onClick={snap}><WandSparkles aria-hidden="true"/>Snap y onto the column space</button><button onClick={()=>{setX1a(1);setX1b(2);setK(3);setYa(5);setYb(2)}}><RotateCcw aria-hidden="true"/>Reset defaults</button></div>
    <div className="u1-lab-grid"><div className="u1-controls">
    ...
    <div className={`u1-observation ${zeroU?"warn":""}`}><Sigma aria-hidden="true"/><p>{zeroU?<><b>u is the zero vector:</b> X has no columns to project onto.</>:eLen<.01?<><b>Perfect fit:</b> y already sits on the column space, so e=0 and there's nothing left to explain.</>:<><b>Orthogonality confirmed:</b> Xᵀe=[{f(cte1,4)}, {f(cte2,4)}] ≈ [0,0] — the dashed residual is exactly perpendicular to the column-space line (see the right-angle mark), whatever y you choose. With a real dataset of many features and rows, the same X⁺y (or (XᵀX)⁻¹Xᵀy) formula does this projection in one step.</>}</p></div>
    <h4 style={{fontSize:"13px",margin:"18px 0 8px",color:"#172033"}}>Bridge to Task 3</h4>
    <div className="u1-calc-table">
      <div style={{gridTemplateColumns:"minmax(0,1.5fr) minmax(0,1.9fr) minmax(0,.5fr)"}}><b>This demo</b><b>Task 3 (your real dataset)</b><b>Marks</b></div>
      <div style={{gridTemplateColumns:"minmax(0,1.5fr) minmax(0,1.9fr) minmax(0,.5fr)"}}><span>Check rank(X) — columns collapse onto one line</span><span>(a) Rank, column space & null space of X</span><span>14</span></div>
      <div style={{gridTemplateColumns:"minmax(0,1.5fr) minmax(0,1.9fr) minmax(0,.5fr)"}}><span>β̂ = X⁺y and ŷ = Xβ̂</span><span>(b) State and apply the projection formula</span><span>12</span></div>
      <div style={{gridTemplateColumns:"minmax(0,1.5fr) minmax(0,1.9fr) minmax(0,.5fr)"}}><span>Compute X⁺, β̂ and ŷ for this 2×2 example</span><span>(c) Compute β̂, ŷ and e on your chosen dataset</span><span>12</span></div>
      <div style={{gridTemplateColumns:"minmax(0,1.5fr) minmax(0,1.9fr) minmax(0,.5fr)"}}><span>Confirm Xᵀe = [0, 0]</span><span>(d) Prove residual orthogonality</span><span>12</span></div>
...
function MissionPanel({ chapter, completed, toggle }: { chapter: Chapter; completed: string[]; toggle: (key: string) => void }) {
  return <section className="u1-missions"><div className="u1-mission-head"><div><Target aria-hidden="true"/><span><b>Experiment missions</b>Do these inside the live lab—then tick only when you can explain what happened.</span></div><strong>{missions[chapter.id].filter((_,i)=>completed.includes(`${chapter.id}-${i}`)).length}/{missions[chapter.id].length}</strong></div><div className="u1-mission-grid">{missions[chapter.id].map((m,i)=>{const key=`${chapter.id}-${i}`,done=completed.includes(key);return <button className={done?"done":""} aria-pressed={done} key={m} onClick={()=>toggle(key)}><i>{done?<Check aria-hidden="true"/>:i+1}</i><span>{m}</span></button>})}</div></section>;
}
...
function PythonPanel({ chapter }: { chapter: Chapter }) {
  ...
  return <section className="u1-python"><div><div><Code2 aria-hidden="true"/><span><b>NumPy connection</b>See how the visual experiment is expressed in Python — this is a worked reference, not a live code runner.</span></div><button onClick={()=>setOpen(v=>!v)}>{open?"Hide code":"Open code lab"}<ChevronDown aria-hidden="true"/></button></div>{open&&<div className="u1-python-body"><div className="u1-code-window"><header><i/><i/><i/><span>unit_1_lab.py</span></header><pre><code>import numpy as np{"\n\n"}{sample.code}</code></pre></div><div className="u1-run-panel"><button onClick={()=>setRan(true)}><Play aria-hidden="true"/>Reveal expected output</button><span>Expected output</span><pre>{ran?sample.output:"Click \"Reveal expected output\" to check your own reasoning against it."}</pre></div></div>}</section>;
}
...
function MisconceptionLab(){
  const[answers,setAnswers]=useState<(boolean|null)[]>(()=>{const saved=readJSON<(boolean|null)[]>("unit1-misconception-answers",[]);return saved.length===misconceptions.length?saved:Array(misconceptions.length).fill(null)});
  useEffect(()=>{writeJSON("unit1-misconception-answers",answers)},[answers]);
  return <section className="u1-mistakes"><div className="u1-mistake-head"><div><CircleHelp aria-hidden="true"/><span><b>Mistake detective</b>Decide whether each common statement is true or false.</span></div><strong>{answers.filter((a,i)=>a===misconceptions[i].truth).length}/{misconceptions.length} correct</strong></div><div className="u1-mistake-grid">{misconceptions.map((m,i)=><article key={m.s}><p>{m.s}</p><div><button className={answers[i]===true?(m.truth?"correct":"wrong"):""} aria-pressed={answers[i]===true} onClick={()=>setAnswers(v=>v.map((a,j)=>j===i?true:a))}>True</button><button className={answers[i]===false?(!m.truth?"correct":"wrong"):""} aria-pressed={answers[i]===false} onClick={()=>setAnswers(v=>v.map((a,j)=>j===i?false:a))}>False</button></div>{answers[i]!==null&&<small>{answers[i]===m.truth?<Check aria-hidden="true"/>:<CircleHelp aria-hidden="true"/>}{m.why}</small>}</article>)}</div></section>;
}
...
function ConceptCard({ chapter }: { chapter: Chapter }){
  ...
  return <section className="u1-concept"><div className="u1-concept-head"><span>{chapter.number}</span><div><p>GUIDED CONCEPT</p><h2>{chapter.title}</h2></div></div><div className="u1-step-tabs">{steps.map((s,i)=>{const I=s.icon;return <button key={s.label} className={step===i?"active":""} aria-pressed={step===i} onClick={()=>setStep(i)}><I aria-hidden="true"/><span>{i+1}. {s.label}</span></button>})}</div><div className="u1-step-body"><span>0{step+1}</span><div><h3>{steps[step].label}</h3><p>{steps[step].text}</p>{step===2&&<aside>Read every symbol from the example before trying to memorise the expression.</aside>}</div></div><div className="u1-check"><div><CircleHelp aria-hidden="true"/><span><b>Quick check</b>{chapter.check.q}</span></div><div className="u1-check-options">{chapter.check.options.map((o,i)=><button className={choice===null?"":i===chapter.check.answer?"correct":choice===i?"wrong":""} aria-pressed={choice===i} key={o} onClick={()=>setChoice(i)}>{choice!==null&&i===chapter.check.answer?<Check aria-hidden="true"/>:<i aria-hidden="true">{String.fromCharCode(65+i)}</i>}{o}</button>)}</div>{choice!==null&&<p className={correct?"correct":"wrong"}>{correct?"Correct. ":"Not quite. "}{chapter.check.why}</p>}</div></section>
}
